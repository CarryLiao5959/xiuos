...
void xWriteCH376Cmd( rt_uint8_t mCmd );  /* 向CH376写命令 */
void xEndCH376Cmd( void );			/* 结束CH376命令,仅用于SPI接口方式 */
void xWriteCH376Data( rt_uint8_t mData );	/* 向CH376写数据 */
rt_uint8_t	xReadCH376Data( void );			/* 从CH376读数据 */
rt_uint8_t	Query376Interrupt( void );		/* 查询CH376中断(INT#引脚为低电平) */
rt_uint8_t	mInitCH376Host( void );			/* 初始化CH376 */
...
typedef struct _FAT_DIR_INFO {
	rt_uint8_t	DIR_Name[11];					/* 00H,文件名,共11字节,不足处填空格 */
	rt_uint8_t	DIR_Attr;						/* 0BH,文件属性,参考后面的说明 */
	rt_uint8_t	DIR_NTRes;						/* 0CH */
	rt_uint8_t	DIR_CrtTimeTenth;				/* 0DH,文件创建的时间,以0.1秒单位计数 */
	rt_uint16_t	DIR_CrtTime;					/* 0EH,文件创建的时间 */
	rt_uint16_t	DIR_CrtDate;					/* 10H,文件创建的日期 */
	rt_uint16_t	DIR_LstAccDate;					/* 12H,最近一次存取操作的日期 */
	rt_uint16_t	DIR_FstClusHI;					/* 14H */
	rt_uint16_t	DIR_WrtTime;					/* 16H,文件修改时间,参考前面的宏MAKE_FILE_TIME */
	rt_uint16_t	DIR_WrtDate;					/* 18H,文件修改日期,参考前面的宏MAKE_FILE_DATE */
	rt_uint16_t	DIR_FstClusLO;					/* 1AH */
	rt_uint32_t	DIR_FileSize;					/* 1CH,文件长度 */
...
typedef struct _BULK_ONLY_CBW {
	rt_uint32_t	CBW_Sig;
	rt_uint32_t	CBW_Tag;
	rt_uint8_t	CBW_DataLen0;					/* 08H,输入: 数据传输长度,对于输入数据其有效值是0到48,对于输出数据其有效值为0到33 */
	rt_uint8_t	CBW_DataLen1;
	rt_uint16_t	CBW_DataLen2;
	rt_uint8_t	CBW_Flag;						/* 0CH,输入: 传输方向等标志,位7为1则输入数据,位为0则输出数据或者没有数据 */
	rt_uint8_t	CBW_LUN;
	rt_uint8_t	CBW_CB_Len;						/* 0EH,输入: 命令块的长度,有效值是1到16 */
	rt_uint8_t	CBW_CB_Buf[16];					/* 0FH,输入: 命令块,该缓冲区最多为16个字节 */
} BULK_ONLY_CBW, *P_BULK_ONLY_CBW;			/* BulkOnly协议的命令块, 输入CBW结构 */

/* INQUIRY命令的返回数据 */
typedef struct _INQUIRY_DATA {
	rt_uint8_t	DeviceType;					/* 00H, 设备类型 */
	rt_uint8_t	RemovableMedia;				/* 01H, 位7为1说明是移动存储 */
	rt_uint8_t	Versions;					/* 02H, 协议版本 */
	rt_uint8_t	DataFormatAndEtc;			/* 03H, 指定返回数据格式 */
	rt_uint8_t	AdditionalLength;			/* 04H, 后续数据的长度 */
	rt_uint8_t	Reserved1;
	rt_uint8_t	Reserved2;
	rt_uint8_t	MiscFlag;					/* 07H, 一些控制标志 */
	rt_uint8_t	VendorIdStr[8];				/* 08H, 厂商信息 */
	rt_uint8_t	ProductIdStr[16];			/* 10H, 产品信息 */
	rt_uint8_t	ProductRevStr[4];			/* 20H, 产品版本 */
} INQUIRY_DATA, *P_INQUIRY_DATA;		/* 24H */

/* REQUEST SENSE命令的返回数据 */
typedef struct _SENSE_DATA {
	rt_uint8_t	ErrorCode;					/* 00H, 错误代码及有效位 */
	rt_uint8_t	SegmentNumber;
	rt_uint8_t	SenseKeyAndEtc;				/* 02H, 主键码 */
	rt_uint8_t	Information0;
	rt_uint8_t	Information1;
	rt_uint8_t	Information2;
	rt_uint8_t	Information3;
	rt_uint8_t	AdditSenseLen;				/* 07H, 后续数据的长度 */
	rt_uint8_t	CmdSpecInfo[4];
	rt_uint8_t	AdditSenseCode;				/* 0CH, 附加键码 */
	rt_uint8_t	AddSenCodeQual;				/* 0DH, 详细的附加键码 */
	rt_uint8_t	FieldReplaUnit;
	rt_uint8_t	SenseKeySpec[3];
...
typedef union _CH376_CMD_DATA {
	struct {
		rt_uint8_t	mBuffer[ MAX_FILE_NAME_LEN ];
	} Default;
	...
	struct {
		rt_uint8_t	mUpdateFileSz;				/* 输入参数: 是否允许更新文件长度, 0则禁止更新长度 */
	} FileCLose;							/* CMD1H_FILE_CLOSE: 关闭当前已经打开的文件 */

	struct {
		rt_uint8_t	mDirInfoIndex;				/* 输入参数: 指定需要读取的目录信息结构在扇区内的索引号, 0FFH则为当前已经打开的文件 */
	} DirInfoRead;							/* CMD1H_DIR_INFO_READ: 读取文件的目录信息 */

	union {
		rt_uint32_t	mByteOffset;				/* 输入参数: 偏移字节数,以字节为单位的偏移量(总长度32位,低字节在前) */
		rt_uint32_t	mSectorLba;					/* 返回: 当前文件指针对应的绝对线性扇区号,0FFFFFFFFH则已到文件尾(总长度32位,低字节在前) */
	} ByteLocate;							/* CMD4H_BYTE_LOCATE: 以字节为单位移动当前文件指针 */

	struct {
		rt_uint16_t	mByteCount;					/* 输入参数: 请求读取的字节数(总长度16位,低字节在前) */
	} ByteRead;								/* CMD2H_BYTE_READ: 以字节为单位从当前位置读取数据块 */

	struct {
		rt_uint16_t	mByteCount;					/* 输入参数: 请求写入的字节数(总长度16位,低字节在前) */
	} ByteWrite;							/* CMD2H_BYTE_WRITE: 以字节为单位向当前位置写入数据块 */

	union {
		rt_uint32_t	mSectorOffset;				/* 输入参数: 偏移扇区数,以扇区为单位的偏移量(总长度32位,低字节在前) */
		rt_uint32_t	mSectorLba;					/* 返回: 当前文件指针对应的绝对线性扇区号,0FFFFFFFFH则已到文件尾(总长度32位,低字节在前) */
	} SectorLocate;							/* CMD4H_SEC_LOCATE: 以扇区为单位移动当前文件指针 */

	struct {
		rt_uint8_t	mSectorCount;				/* 输入参数: 请求读取的扇区数 */
											/* 返回: 允许读取的扇区数 */
		rt_uint8_t	mReserved1;
		rt_uint8_t	mReserved2;
		rt_uint8_t	mReserved3;
		rt_uint32_t	mStartSector;				/* 返回: 允许读取的扇区块的起始绝对线性扇区号(总长度32位,低字节在前) */
	} SectorRead;							/* CMD1H_SEC_READ: 以扇区为单位从当前位置读取数据块 */

	struct {
		rt_uint8_t	mSectorCount;				/* 输入参数: 请求写入的扇区数 */
											/* 返回: 允许写入的扇区数 */
		rt_uint8_t	mReserved1;
		rt_uint8_t	mReserved2;
		rt_uint8_t	mReserved3;
		rt_uint32_t	mStartSector;				/* 返回: 允许写入的扇区块的起始绝对线性扇区号(总长度32位,低字节在前) */
	} SectorWrite;							/* CMD1H_SEC_WRITE: 以扇区为单位在当前位置写入数据块 */

	struct {
		rt_uint32_t	mDiskSizeSec;				/* 返回: 整个物理磁盘的总扇区数(总长度32位,低字节在前) */
	} DiskCapacity;							/* CMD0H_DISK_CAPACITY: 查询磁盘物理容量 */

	struct {
		rt_uint32_t	mTotalSector;				/* 返回: 当前逻辑盘的总扇区数(总长度32位,低字节在前) */
		rt_uint32_t	mFreeSector;				/* 返回: 当前逻辑盘的剩余扇区数(总长度32位,低字节在前) */
		rt_uint8_t	mDiskFat;					/* 返回: 当前逻辑盘的FAT类型,1-FAT12,2-FAT16,3-FAT32 */
	...
	struct {
		rt_uint8_t	mMaxLogicUnit;				/* 返回: USB存储器的最大逻辑单元号 */
	...
	struct {
		rt_uint32_t	mDiskSizeSec;				/* 返回: 整个物理磁盘的总扇区数(总长度32位,高字节在前) */
	} DiskSize;								/* CMD0H_DISK_SIZE: 获取USB存储器的容量 */

	struct {
		rt_uint32_t	mStartSector;				/* 输入参数: LBA扇区地址(总长度32位,低字节在前) */
		rt_uint8_t	mSectorCount;				/* 输入参数: 请求读取的扇区数 */
