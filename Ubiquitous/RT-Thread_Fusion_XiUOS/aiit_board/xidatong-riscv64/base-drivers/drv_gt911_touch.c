...
static rt_size_t gt911_read_point(struct rt_touch_device *touch, void *buf, rt_size_t read_num)
{
    ...
    {   
        LOG_D("read point: data is not ready\n");
        read_num = 0;
        goto exit_;
    }
    ...
    {   
        LOG_D("read point: point num is not correct\n");
        read_num = 0;
        goto exit_;
    }
...
int rt_hw_gt911_init(const char *name, struct rt_touch_config *cfg)
{
    ...
    rt_hw_touch_register(touch_device, name, RT_DEVICE_FLAG_RDONLY, RT_NULL);
