...
rt_uint8_t	CH376ReadBlock( PUINT8 buf )  /* 从当前主机端点的接收缓冲区读取数据块,返回长度 */
{
	rt_uint8_t	s, l;
...
rt_uint8_t	CH376WriteReqBlock( PUINT8 buf )  /* 向内部指定缓冲区写入请求的数据块,返回长度 */
{
	rt_uint8_t	s, l;
...
void CH376WriteHostBlock( PUINT8 buf, rt_uint8_t len )  /* 向USB主机端点的发送缓冲区写入数据块 */
{
...
void CH376WriteOfsBlock( PUINT8 buf, rt_uint8_t ofs, rt_uint8_t len )  /* 向内部缓冲区指定偏移地址写入数据块 */
{
	...
	if ( xReadCH376Data( ) < 0x43 ) {
		if ( CH376ReadVar8( VAR_DISK_STATUS ) < DEF_DISK_READY ) {
			...
			if ( s == USB_INT_SUCCESS ) {
				...
				if ( s ) {
					CH376WriteVar32( 0x4C, CH376ReadVar32( 0x4C ) + ( (rt_uint16_t)s << 8 ) );
					CH376WriteVar32( 0x50, CH376ReadVar32( 0x50 ) + ( (rt_uint16_t)s << 8 ) );
...
rt_uint32_t	CH376Read32bitDat( void )  /* 从CH376芯片读取32位的数据并结束命令 */
{
	rt_uint8_t	c0, c1, c2, c3;
	c0 = xReadCH376Data( );
	c1 = xReadCH376Data( );
	c2 = xReadCH376Data( );
	c3 = xReadCH376Data( );
	xEndCH376Cmd( );
	return( c0 | (rt_uint16_t)c1 << 8 | (rt_uint32_t)c2 << 16 | (rt_uint32_t)c3 << 24 );
}

rt_uint8_t	CH376ReadVar8( rt_uint8_t var )  /* 读CH376芯片内部的8位变量 */
{
	rt_uint8_t	c0;
...
void	CH376WriteVar8( rt_uint8_t var, rt_uint8_t dat )  /* 写CH376芯片内部的8位变量 */
{
...
rt_uint32_t	CH376ReadVar32( rt_uint8_t var )  /* 读CH376芯片内部的32位变量 */
{
...
void	CH376WriteVar32( rt_uint8_t var, rt_uint32_t dat )  /* 写CH376芯片内部的32位变量 */
{
	xWriteCH376Cmd( CMD50_WRITE_VAR32 );
	xWriteCH376Data( var );
	xWriteCH376Data( (rt_uint8_t)dat );
	xWriteCH376Data( (rt_uint8_t)( (rt_uint16_t)dat >> 8 ) );
	xWriteCH376Data( (rt_uint8_t)( dat >> 16 ) );
	xWriteCH376Data( (rt_uint8_t)( dat >> 24 ) );
	xEndCH376Cmd( );
}
...
rt_uint32_t	CH376GetFileSize( void )  /* 读取当前文件长度 */
{
	return( CH376ReadVar32( VAR_FILE_SIZE ) );
}

rt_uint8_t	CH376GetDiskStatus( void )  /* 获取磁盘和文件系统的工作状态 */
{
	return( CH376ReadVar8( VAR_DISK_STATUS ) );
}

rt_uint8_t	CH376GetIntStatus( void )  /* 获取中断状态并取消中断请求 */
{
	rt_uint8_t	s;
...
rt_uint8_t	Wait376Interrupt( void )  /* 等待CH376中断(INT#低电平)，返回中断状态码, 超时则返回ERR_USB_UNKNOWN */
{
...
rt_uint8_t	CH376SendCmdWaitInt( rt_uint8_t mCmd )  /* 发出命令码后,等待中断 */
{
	xWriteCH376Cmd( mCmd );
	xEndCH376Cmd( );
...
rt_uint8_t	CH376SendCmdDatWaitInt( rt_uint8_t mCmd, rt_uint8_t mDat )  /* 发出命令码和一字节数据后,等待中断 */
{
	xWriteCH376Cmd( mCmd );
...
rt_uint8_t	CH376DiskReqSense( void )  /* 检查USB存储器错误 */
{
	rt_uint8_t	s;
	rt_thread_mdelay(5);
...
rt_uint8_t	CH376DiskConnect( void )  /* 检查U盘是否连接,不支持SD卡 */
{
	if ( Query376Interrupt( ) ) CH376GetIntStatus( );  /* 检测到中断 */
	return( CH376SendCmdWaitInt( CMD0H_DISK_CONNECT ) );
}

rt_uint8_t	CH376DiskMount( void )  /* 初始化磁盘并测试磁盘是否就绪 */
{
	return( CH376SendCmdWaitInt( CMD0H_DISK_MOUNT ) );
}

rt_uint8_t	CH376FileOpen( PUINT8 name )  /* 在根目录或者当前目录下打开文件或者目录(文件夹) */
{
...
rt_uint8_t	CH376FileCreate( PUINT8 name )  /* 在根目录或者当前目录下新建文件,如果文件已经存在那么先删除 */
{
	if ( name ) CH376SetFileName( name );  /* 设置将要操作的文件的文件名 */
	return( CH376SendCmdWaitInt( CMD0H_FILE_CREATE ) );
}

rt_uint8_t	CH376DirCreate( PUINT8 name )  /* 在根目录下新建目录(文件夹)并打开,如果目录已经存在那么直接打开 */
{
	CH376SetFileName( name );  /* 设置将要操作的文件的文件名 */
#ifndef	DEF_IC_V43_U
	if ( name[0] == DEF_SEPAR_CHAR1 || name[0] == DEF_SEPAR_CHAR2 ) CH376WriteVar32( VAR_CURRENT_CLUST, 0 );
...
rt_uint8_t	CH376SeparatePath( PUINT8 path )  /* 从路径中分离出最后一级文件名或者目录(文件夹)名,返回最后一级文件名或者目录名的字节偏移 */
{
	PUINT8	pName;
...
rt_uint8_t	CH376FileOpenDir( PUINT8 PathName, rt_uint8_t StopName )  /* 打开多级目录下的文件或者目录的上级目录,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
...
rt_uint8_t	CH376FileOpenPath( PUINT8 PathName )  /* 打开多级目录下的文件或者目录(文件夹),支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
{
	return( CH376FileOpenDir( PathName, 0xFF ) );
}

rt_uint8_t	CH376FileCreatePath( PUINT8 PathName )  /* 新建多级目录下的文件,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
{
	rt_uint8_t	s;
	rt_uint8_t	Name;
...
#ifdef	EN_DIR_CREATE
rt_uint8_t	CH376DirCreatePath( PUINT8 PathName )  /* 新建多级目录下的目录(文件夹)并打开,支持多级目录路径,支持路径分隔符,路径长度不超过255个字符 */
{
	rt_uint8_t	s;
	rt_uint8_t	Name;
	rt_uint8_t	ClustBuf[4];
...
rt_uint8_t	CH376FileErase( PUINT8 PathName )  /* 删除文件,如果已经打开则直接删除,否则对于文件会先打开再删除,支持多级目录路径 */
{
	rt_uint8_t	s;
	if ( PathName ) {  /* 文件尚未打开 */
...
rt_uint8_t	CH376FileClose( rt_uint8_t UpdateSz )  /* 关闭当前已经打开的文件或者目录(文件夹) */
{
	return( CH376SendCmdDatWaitInt( CMD1H_FILE_CLOSE, UpdateSz ) );
}

rt_uint8_t	CH376DirInfoRead( void )  /* 读取当前文件的目录信息 */
{
	return( CH376SendCmdDatWaitInt( CMD1H_DIR_INFO_READ, 0xFF ) );
}

rt_uint8_t	CH376DirInfoSave( void )  /* 保存文件的目录信息 */
{
	return( CH376SendCmdWaitInt( CMD0H_DIR_INFO_SAVE ) );
}

rt_uint8_t	CH376ByteLocate( rt_uint32_t offset )  /* 以字节为单位移动当前文件指针 */
{
	xWriteCH376Cmd( CMD4H_BYTE_LOCATE );
	xWriteCH376Data( (rt_uint8_t)offset );
	xWriteCH376Data( (rt_uint8_t)((rt_uint16_t)offset>>8) );
	xWriteCH376Data( (rt_uint8_t)(offset>>16) );
	xWriteCH376Data( (rt_uint8_t)(offset>>24) );
	xEndCH376Cmd( );
	return( Wait376Interrupt( ) );
}

rt_uint8_t	CH376ByteRead( PUINT8 buf, rt_uint16_t ReqCount, PUINT16 RealCount )  /* 以字节为单位从当前位置读取数据块 */
{
	rt_uint8_t	s;
	xWriteCH376Cmd( CMD2H_BYTE_READ );
	xWriteCH376Data( (rt_uint8_t)ReqCount );
	xWriteCH376Data( (rt_uint8_t)(ReqCount>>8) );
	xEndCH376Cmd( );
...
rt_uint8_t	CH376ByteWrite( PUINT8 buf, rt_uint16_t ReqCount, PUINT16 RealCount )  /* 以字节为单位向当前位置写入数据块 */
{
	rt_uint8_t	s;
...
rt_uint8_t	CH376DiskCapacity( PUINT32 DiskCap )  /* 查询磁盘物理容量,扇区数 */
{
	rt_uint8_t	s;
...
rt_uint8_t	CH376DiskQuery( PUINT32 DiskFre )  /* 查询磁盘剩余空间信息,扇区数 */
{
	rt_uint8_t	s;
	rt_uint8_t	c0, c1, c2, c3;
#ifndef	DEF_IC_V43_U
	...
	if ( s == USB_INT_SUCCESS ) {  /* 参考CH376INC.H文件中CH376_CMD_DATA结构的DiskQuery */
		...
		c2 = xReadCH376Data( );
		c3 = xReadCH376Data( );
		*DiskFre = c0 | (rt_uint16_t)c1 << 8 | (rt_uint32_t)c2 << 16 | (rt_uint32_t)c3 << 24;
...
rt_uint8_t	CH376SecLocate( rt_uint32_t offset )  /* 以扇区为单位移动当前文件指针 */
{
...
rt_uint8_t	CH376DiskReadSec( PUINT8 buf, rt_uint32_t iLbaStart, rt_uint8_t iSectorCount )  /* 从U盘读取多个扇区的数据块到缓冲区,不支持SD卡 */
/* iLbaStart 是准备读取的线性起始扇区号, iSectorCount 是准备读取的扇区数 */
{
	rt_uint8_t	s, err;
	rt_uint16_t	mBlockCount;
	for ( err = 0; err != 3; ++ err ) {  /* 出错重试 */
		xWriteCH376Cmd( CMD5H_DISK_READ );  /* 从USB存储器读扇区 */
		xWriteCH376Data( (rt_uint8_t)iLbaStart );  /* LBA的最低8位 */
		xWriteCH376Data( (rt_uint8_t)( (rt_uint16_t)iLbaStart >> 8 ) );
		xWriteCH376Data( (rt_uint8_t)( iLbaStart >> 16 ) );
		xWriteCH376Data( (rt_uint8_t)( iLbaStart >> 24 ) );  /* LBA的最高8位 */
...
rt_uint8_t	CH376DiskWriteSec( PUINT8 buf, rt_uint32_t iLbaStart, rt_uint8_t iSectorCount )  /* 将缓冲区中的多个扇区的数据块写入U盘,不支持SD卡 */
...
rt_uint8_t	CH376SecRead( PUINT8 buf, rt_uint8_t ReqCount, PUINT8 RealCount )  /* 以扇区为单位从当前位置读取数据块,不支持SD卡 */
{
	rt_uint8_t	s;
	rt_uint8_t	cnt;
	rt_uint32_t	StaSec;
#ifndef	DEF_IC_V43_U
	rt_uint32_t	fsz, fofs;
#endif
	if ( RealCount ) *RealCount = 0;
	do {
#ifndef	DEF_IC_V43_U
		...
		if ( cnt == 0x41 ) {
			...
			xReadCH376Data( );
			fsz = xReadCH376Data( );
			fsz |= (rt_uint16_t)(xReadCH376Data( )) << 8;
			cnt = xReadCH376Data( );
			fsz |= (rt_uint32_t)cnt << 16;
			xWriteCH376Cmd( CMD14_READ_VAR32 );
			xWriteCH376Data( VAR_CURRENT_OFFSET );
			xReadCH376Data( );
			fofs = xReadCH376Data( );
			fofs |= (rt_uint16_t)(xReadCH376Data( )) << 8;
			fofs |= (rt_uint32_t)(xReadCH376Data( )) << 16;
...
rt_uint8_t	CH376SecWrite( PUINT8 buf, rt_uint8_t ReqCount, PUINT8 RealCount )  /* 以扇区为单位在当前位置写入数据块,不支持SD卡 */
{
	rt_uint8_t	s;
	rt_uint8_t	cnt;
	rt_uint32_t	StaSec;
...
rt_uint8_t	CH376LongNameWrite( PUINT8 buf, rt_uint16_t ReqCount )  /* 长文件名专用的字节写子程序 */
{
	rt_uint8_t	s;
#ifndef	DEF_IC_V43_U
	rt_uint8_t	c;
...
rt_uint8_t	CH376CheckNameSum( PUINT8 DirName )  /* 计算长文件名的短文件名检验和,输入为无小数点分隔符的固定11字节格式 */
{
	rt_uint8_t	NameLen;
	rt_uint8_t	CheckSum;
	CheckSum = 0;
...
rt_uint8_t	CH376LocateInUpDir( PUINT8 PathName )  /* 在上级目录(文件夹)中移动文件指针到当前文件目录信息所在的扇区 */
...
rt_uint8_t	CH376GetLongName( PUINT8 PathName, PUINT8 LongName )  /* 由短文件名或者目录(文件夹)名获得相应的长文件名 */
...
{
	rt_uint8_t	s;
	rt_uint16_t	NameCount;	/* 长文件名字节计数 */
	s = CH376FileOpenPath( PathName );  /* 打开多级目录下的文件或者目录 */
...
rt_uint8_t	CH376CreateLongName( PUINT8 PathName, PUINT8 LongName )  /* 新建具有长文件名的文件,关闭文件后返回,LongName输入路径必须在RAM中 */
...
{
	rt_uint8_t	s, i;
	rt_uint8_t	DirBlockCnt;	/* 长文件名占用文件目录结构的个数 */
	rt_uint16_t	count;			/* 临时变量,用于计数,用于字节读文件方式下实际读取的字节数 */
	rt_uint16_t	NameCount;		/* 长文件名字节计数 */
	rt_uint32_t	NewFileLoc;		/* 当前文件目录信息在上级目录中的起始位置,偏移地址 */
